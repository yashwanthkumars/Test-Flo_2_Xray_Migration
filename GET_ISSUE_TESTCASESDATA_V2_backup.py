...
JQL_QUERY = 'project = GreyMatter  AND issuetype  =  "Test Plan"'
...
def main():
    ...
    logger.info("Starting Test Plan Data Extraction")
    ...
    for plan_idx, plan in enumerate(test_plans, 1):
        ...
        logger.info(f"Processing {plan_key} with {len(subtasks)} subtasks")
 
        for sub_idx, sub in enumerate(subtasks, 1):
            sub_key = sub["key"]
            sub_id = sub["id"]
    ...
    # Write to CSV
    csv_file = "jira_test_Plan_with_casesGrayMatter.csv"
